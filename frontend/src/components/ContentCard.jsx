...
const ContentCard = ({ content }) => {
  ...
  if (isFree) {
    actionText = content.contentType === 'blog' ? 'Leer artículo' : 'Ver ahora';
    handleAction = (content.contentType === 'blog' || content.contentType === 'videoteca')
      ? navigateToDetail
      : () => {
          if (!user) {
            alert('Por favor, inicia sesión para acceder a Cursos y Workshops.');
            navigate('/login');
            return;
          }
          alert(`[Acceso Concedido] Reproduciendo curso gratuito: "${content.title}"`);
        };
    ...
    if (isSubscribed) {
      ...
      handleAction = (content.contentType === 'blog' || content.contentType === 'videoteca')
        ? navigateToDetail
        : () => alert(`[Acceso Premium Concedido] Reproduciendo: "${content.title}"`);
    } else {
    ...
    if (isOwned) {
      ...
      handleAction = (content.contentType === 'blog' || content.contentType === 'videoteca')
        ? navigateToDetail
        : () => alert(`[Acceso de Compra Concedido] Reproduciendo: "${content.title}"`);
    } else {
  ...
  const hasAccess = (isFree && (!isCourseOrWorkshop || !!user)) || (content.accessType === 'subscription' && isSubscribed) || (content.accessType === 'one-time-purchase' && isOwned);
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          <div>
            ...
            {showCardImage && (
              <div
                onClick={(content.contentType === 'blog' || content.contentType === 'videoteca') && hasAccess ? navigateToDetail : handleAction}
                style={{
                  width: '100%',
                  height: '180px',
        ...
        {/* 2. Interactive Hover Overlay (Modal-like details) */}
        <div style={{
          position: 'absolute',
          top: 0,
          left: 0,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px', minHeight: '28px' }}>
              {isCourseOrWorkshop ? (
                ...
                >
                  <IoFolderOpen size={14} style={{ marginRight: '2px' }} /> Guardar
                </button>
              )}
              <span style={{ fontSize: '11px', color: 'var(--gray-500)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '1px' }}>
                {content.accessType === 'subscription' && 'Membresía'}
                {content.accessType === 'one-time-purchase' && 'Pago Único'}
              </span>
            </div>
            ...
            {/* Full Scrollable Description inside hover modal */}
            <div style={{
              maxHeight: '180px',
              overflowY: 'auto',
              color: 'var(--white)',
